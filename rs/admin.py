...
#def mark_photos_as_approved(request, is_private=False, bookmark = ''):
    ## this is a maintenance program, that will run through all uploaded photos (marked as public) in the system for
    ## and mark them as having been approved. - will only be used once..
    ##
    #PAGESIZE = 8 # total number of photos per page (how many to fetch in the query)
    #PAGEWIDTH = 8 # number of photos per row
    ...
    #query_filter_dict['is_approved'] = False
        ...
        #bookmark_key = db.Key(bookmark)
        #photo_bookmark_object = db.get(bookmark_key)
        #query_filter_dict['creation_date <= '] =  photo_bookmark_object.creation_date # only get the profiles that are newer than the bookmark.
    ...
    #for photo_object in batch[:PAGESIZE]:  
        ...
        #profile_href = "/rs/other/%s/" % str(photo_parentobject.key())
        #url_for_photo = '/rs/ajax/get_%s_photo/%s.png' % ("medium", photo_object_key_str)
    ...
    #html_head = "<html>"
    #if len(batch) == PAGESIZE + 1:
        #generated_html += "Processing!<br><br>"
        
        #bookmark = str(batch[-1].key())
        
        #continue_html += '<a href=/rs/admin/approve_public_photos_bookmark/%s/>Click to continue processing</a><br>\n' % bookmark
       
        #html_head = """
        #<html>
        #<head>
          #<meta http-equiv="refresh" content="0;url=/rs/admin/approve_public_photos_bookmark/%s/"/>  
        #</head>""" % bookmark
    #else:
        #generated_html += "DONE!<br><br>"
    
    #user = users.get_current_user()
    
    #if user:
        #generated_html += '<a href=/rs/admin/review_public_photos>Review public photos</a><br>\n'    
        #generated_html += '<a href=/rs/admin/review_private_photos>Review private photos</a><br>\n'       
        #generated_html += '<br>Welcome %s: <a href=\"%s\">sign out</a><br>' % (user.nickname(), users.create_logout_url("/"))
    #else:
        #generated_html += "Error: user not logged in!!!!"
        
    #html_tail = "</html>"
    
    #html_to_render = html_head + continue_html + generated_html + html_tail
    #return HttpResponse(html_to_render)
