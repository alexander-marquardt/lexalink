...
def get_my_internal_advertisements(additional_ads_to_append = []):
...
def render_main_html(request, generated_html, userobject = None, link_to_hide = '', 
    ...
    try:
        ...
        if enable_ads:
            if settings.BUILD_NAME == "Discrete":
                #advertising_info.bottom_banner = get_horizontal_bottom_banner_ad(request)
                #advertising_info.ashley_madison_ad = get_ashley_madison_ad(request)
                #(advertising_info.external_advertisement, advertising_info.is_template_or_code) =  get_side_skyscaper_ad(request)
                (advertising_info.external_advertisement, advertising_info.is_template_or_code) = (None, None)
            else:
                advertising_info.bottom_banner = ''
