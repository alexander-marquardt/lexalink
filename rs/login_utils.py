...
def verify_user_login(request, login_dict):
    ...
    try:
        ...
        <strong>info@%(app_name)s.com</strong>.<p><p><p><p><p><p><p><p></div>
...
def store_friend_registration_tracker(userobject, referring_userobject):

    # This user has been referred to sign-up by another user. We must store information so that we can credit the user that
    # has invited this user.
    # This function will verify that the invited user is not the same person who invited this user to sign up (different IP address,
    # and whatever else we might be able to think of)
    #
    # referring_code: is the obfuscated username of the user that has invited this user to sign up
    #
    # returns True if the referrer should be credited, and False if the referrer profile should not be credited for the new signup.
    ...
    try:
        ...
        friend_registration_tracker = models.FriendRegsitrationTracker()

        if not settings.DEBUG:
            if remoteip == referring_userobject.registration_ip_address or remoteip == referring_userobject.last_login_ip_address:
                # this referring user is not credited, since the new user has registered from the same address that the referer used
                credit_referer = False
                denial_reason = "%s not credited for referring %s due to matching last_login_ip_address or registration_ip_address %s" % (referring_userobject.username, userobject.username, remoteip)
                logging.info(denial_reason)
                friend_registration_tracker.reason_for_credit_denial = denial_reason
                
            elif previous_signup_already_credited_from_invitee_ip_address(remoteip):
                # The referring user is not credited, since the address that the new user is signing up from has already been used for a signup that 
                # already credited a user account.  We (currently) only credit *one* signup per IP address. 
                credit_referer = False
                denial_reason = "%s not credited for referring %s due to previous referral signup on ip %s" % (referring_userobject.username, userobject.username, remoteip)
                logging.info(denial_reason)
                friend_registration_tracker.reason_for_credit_denial = denial_reason
            

        friend_registration_tracker.ip_address_of_invitee = remoteip
            
        friend_registration_tracker.referring_profile_ref = referring_userobject
        friend_registration_tracker.referring_profile_name = referring_userobject.username
        
        friend_registration_tracker.invitee_profile_ref = userobject
        friend_registration_tracker.invitee_profile_name = userobject.username
        
        friend_registration_tracker.referral_credit_awarded = credit_referer

        logging.info("Storing a referral from %s to %s" % (referring_userobject.username, userobject.username))        
        friend_registration_tracker.put()
    
    except:
        error_reporting.log_exception(logging.critical) 
        credit_referer = False

    return credit_referer