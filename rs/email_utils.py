...
def send_vip_invitiation_email(request):
    ...
    info_message = ''
    try:   
        username = email_address = secret_verification_code = remoteip = lang_code =  None
        if request.method == 'POST':
            username = request.POST.get('username',None)
            email_address = request.POST.get('email_address', None)
            lang_code = request.POST.get('lang_code', None)
             
        if not username or not email_address or not lang_code:    
            error_message = u"username: %s email_address: %s lang_code: %s" % (username, email_address, lang_code)
            error_reporting.log_exception(logging.critical, error_message = error_message)   
            # return a non-errorr http response, so that this will not be re-queued -- this error will not resolve itself.
            return http.HttpResponse(error_message)
        ...
        message.to = u"<%s>" % (email_address)
        
        message.html = u"<p>%s</p>" % ugettext("You can personalize and forward this message (or put it on facebook, twitter, etc.) so that both you and your friends \
        can earn VIP credits in %(app_name)s.") % {'app_name': settings.APP_NAME}
        message.html += u"------------------------------"
        message.html += u"<p>%s,</p>" % ugettext("Dear Friend") 
        message.html += u"<p>%s</p>" % \
               ugettext("I am a member of %(app_name)s.com and thought that you might be interested in trying it out. It is a \
               free %(site_description)s site that I have used to meet many interesting people.") % \
               {'app_name' : settings.APP_NAME, 'site_description': constants.type_of_site_for_vip_invite[settings.BUILD_NAME] }
        
        message.html += u"<p>%s</p>" % ugettext("""If you click on the link below and decide to sign up, both you and I will earn %(value)s credit which \
        is enough credit for VIP status (VIP status gives extra message quota and has other benefits).""") % \
            {'value' : u"%s€" % constants.reverse_lookup_client_paid_status_num_credits_awarded_for_euros[constants.CREDITS_AWARDED_FOR_A_REFERRAL]}
                                                                                                           
        link = u"http://%(http_host)s/%(lang_code)s/vip-invite/%(username_obfuscated)s/" % \
                     {'http_host' : request.META['HTTP_HOST'], 'lang_code' : lang_code, 'username_obfuscated' : username_obfuscated,}
        
        message.html += u"<p>%s</p>" % "<a href=%(link)s>%(link)s</a>" % {'link' : link}

        message.html += u"<p>%s</p>" % ugettext("Kind Regards")
        ...
        error_message = u"Unable to send vip-invite email. info_message: %s" % info_message
        error_reporting.log_exception(logging.critical, error_message = error_message)
        return_val =  http.HttpResponseServerError(error_message)    
