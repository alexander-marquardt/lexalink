...
def split_locale_from_request(request):
...
def locale_path(path, locale=''):
    ...
    if localeurl.settings.URL_TYPE != 'path_prefix':
        return path
    # commented out the following, since we want a more orderly fallback to languages based on browser settings, cookies, etc. 
    #if not locale:
        #locale = supported_language(settings.LANGUAGE_CODE)
    if not locale: # new code
